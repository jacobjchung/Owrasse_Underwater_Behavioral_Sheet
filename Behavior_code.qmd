...
NM_in_near <- yearly_totals |>
  group_by(Year) |> #Focusing on per year
  summarise(
    #Get the total amount of time that the NM was recorded
    total_time = round(sum(`# NM in` + `# NM near` + `# NM far`), digits = 4),
    #Get the proportion of time the NM was in his nest.
    NM_pro_in = round((`# NM in`/total_time)*100, digits = 4),
    #Get the proportion of time the NM was near his nest.
    NM_pro_near = round((`# NM near`/total_time)*100, digits = 4),
    #Get the propotion of time the NM was far from his nest
    NM_pro_far = round((`# NM far`/total_time)*100, digits = 4))

NM_in_near.long <- NM_in_near %>% 
  pivot_longer(cols = -c(Year, total_time),
               names_to = "distance",
               values_to = "perc_time")

NM_in_near.long %>% 
  ggplot(aes(Year, perc_time, fill = distance)) +
  geom_col()
```
