...
##### This is the exploratory linear regression model

```{r}
#Total spawns as a function of total female visits
f.tspawns.tvisits.nbmdl <- 
      ...
      file = 'Exploratory model outputs/f.tspawns.tvisits.nbmdl') #Saves the model as an object for outputting later

preds <- predict_response(f.tspawns.tvisits.nbmdl)
plot(preds, show_data = TRUE)

pp_check(f.tspawns.tvisits.nbmdl)

summary(f.tspawns.tvisits.nbmdl) #Gives us model summary
plot(f.tspawns.tvisits.nbmdl) #plots the Markov chains
```

Given our model, we can suggest that for every female visit, there is a 0.07 increase in the total spawns at that nest. Our 95% credibility interval gives us 0.06-0.08, allowing us to suggest that this effect is different from zero. This matches our expectation because with every visit to a nest, we would expect a general increase in the total number of spawns at a nest (however not complete because remember, some females don't spawn).
...
```{r}
snkint <- behavior_sheet_cln |>
  mutate('snk.int' = (nm_aggression_to_snk + sat_aggression_to_snk + snk_submission_to_nm + snk_submission_to_sat)) |>
  ggplot(aes(snk.int, total_female_visits)) +
  geom_point() + 
  geom_smooth()
...
### Building/looking at our 'final' model.

Overarching question: How does the social environment influence female mate choice?

target outcome: How many females decided to spawn

Thus total_female_spawning is the our response variable, because it tells us how many females made the choice to spawn.

Based on our current understanding (and data), what has *direct* effects on this response?

1.  Average number of sneaker
2.  Social interactions at the nest
    1.  NM to SAT
    2.  NM to SNK
    3.  SAT to SNK
    4.  NM to F
...
How are these factors mentioned in our data.

1.  average_snk
2.  Social interactions at the nest
    1.  female_visits_by_sat_passed_to_nm, female_spawning_sat_passed_to_nm, female_spawning_sat, nm_aggression_to_sat, sat_submission_to_nm
    2.  nm_aggression_to_snk, snk_submission_to_nm
    3.  sat_aggression_to_snk, snk_submission_to_sat
    4.  female_visits_by_nm, female_spawning_with_nm_brought_by_nm, nm_aggression_to_female_towards_nest, nm_aggression_to_female_away_from_nest
    5.  female_visit_by_sat, female_spawning_sat_passed_to_nm_but_sat_only_spawn, females_spawing_sat_brought_sat_only_spawn, sat_aggression_to_female_towards_nest

**Note:** It is likely that nm_aggression_to_snk, snk_submission_to_nm, sat_aggression_to_snk, snk_submission_to_sat would be statistically the same as average_snk as each one of those are a related to how many sneakers are present.

### Linear regression models

total_female_spawning \~ average_snk

total_female_spawning \~ female_visits_by_sat_passed_to_nm

total_female_spawning \~ female_spawning_sat_passed_to_nm

total_female_spawning \~ female_spawning_sat

total_female_spawning \~ nm_aggression_to_sat

total_female_spawning \~ sat_submission_to_nm

total_female_spawning \~ nm_aggression_to_snk

total_female_spawning \~ snk_submission_to_nm

total_female_spawning \~ sat_aggression_to_snk

total_female_spawning \~ snk_submission_to_sat

total_female_spawning \~ female_visits_by_nm

total_female_spawning \~ female_spawning_with_nm_brought_by_nm

total_female_spawning \~ nm_aggression_to_female_towards_nest

total_female_spawning \~ nm_aggression_to_female_away_from_nest

total_female_spawning \~ female_visit_by_sat

total_female_spawning \~ female_spawning_sat_passed_to_nm_but_sat_only_spawn

total_female_spawning \~ females_spawing_sat_brought_sat_only_spawn

total_female_spawning \~ sat_aggression_to_female_towards_nest

However, these models are unrealistic, since it is highly unlikely that each one of these factors influence female spawning decision alone. Thus it is important for us to

### Multiple Regression models

ggcore? to look at how variables covary?

For sneaker related models

total_female_spawning \~ nm_aggression_to_snk + snk_submission_to_nm + sat_aggression_to_snk + snk_submission_to_sat

Note: The associated aggression and submission variables (nm and sat respectively) are likely to be redundant since submission is a response to aggression, thus it is likely that both might not be needed. We will run both models to check.

For Nesting Male/SAT related models

total_female_spawning \~ female_visits_by_sat_passed_to_nm + female_spawning_sat_passed_to_nm + female_spawning_sat +nm_aggression_to_sat + total_female_spawning \~ sat_submission_to_nm

For NM to F

total_female_spawning \~ female_visits_by_nm + female_spawning_with_nm_brought_by_nm + nm_aggression_to_female_towards_nest + nm_aggression_to_female_away_from_nest

For SAT to F

total_female_spawning \~ female_visit_by_sat + female_spawning_sat_passed_to_nm_but_sat_only_spawn + females_spawing_sat_brought_sat_only_spawn + sat_aggression_to_female_towards_nest

#### After meeting with Malin

The multiple regression model(s) should be a binomial with the N being the number of female visits, and p being the probability of spawning given a visit. We should normalize the pathways (variables) before putting them in the model, meaning take each variable as a fraction of visits for each pathways, i.e. fraction of females that spawned which were brought by the SAT, fraction of females that spawned which were brought by the SAT and passed to the NM.

To-Dos:

Look at the data distribution for the effect of sneakers on female visits and female spawning. Biologically, quadratic makes sense (low sneakers = low nest success = female doesn't visit/spawn, medium sneakers = moderate nest success = female likely visits/spawns, high sneakers = high nest success = females don't spawn due to high sneaker numbers) but data might show otherwise.

Normalize the variables that we think are important, "female_spawns_by_nm, female_spawns_by_sat, female_spawns_by_sat_passed_to_nm" by the total number of visits

```{r eval = FALSE}
#bf(spawns|trails(visits) ~ 1 + )
...
fv.avgsnkr1 <-
  brm(data = behavior_sheet_cln, #Give the model the tibble
      ...
      iter = 2000, warmup = 1000, chains = 4, cores = 4, #Markov chain parameters
      seed = 4, #Determines which random numbers
      file = 'Data Analysis Outputs/fv.avgsnkr1') #saves it as an output
...
fs.avgsnkr <-
  brm(data = behavior_sheet_cln, #Give the model the tibble
      ...
      iter = 2000, warmup = 1000, chains = 4, cores = 4, #Markov chain parameters
      seed = 4, #Determines which random numbers
      file = 'Data Analysis Outputs/fs.avgsnkr') #saves it as an output
...
#### Varying Pathway's success rates

###### This is scratch code for figuring out how to filter out inconsistent data

```{r}
nm.brought.path <- behavior_sheet_cln |>
  mutate(
    'prob.of.spawn' = ((female_spawning_with_nm_brought_by_nm + female_spawning_with_only_sat_brought_by_nm)/female_visits_by_nm)
  ) |>
  filter_out(prob.of.spawn > 1) |>
  ggplot(aes(female_visits_by_nm, prob.of.spawn)) +
  geom_point(na.rm = T)
...
```{r}
sat.brought.path <- behavior_sheet_cln |>
  mutate(
    'prob.of.spawn' = ((female_spawning_sat_brought + females_spawing_sat_brought_sat_only_spawn)/female_visit_by_sat)
  ) |>
  filter_out(prob.of.spawn > 1) |>
  ggplot(aes(female_visit_by_sat, prob.of.spawn)) +
  geom_point(na.rm = T)
...
```{r}
sat.pass.brought.path <- behavior_sheet_cln |>
  mutate(
    'prob.of.spawn' = (female_spawning_sat_passed_to_nm + female_spawning_sat_passed_to_nm_but_only_sat_spawn)/(female_visits_by_sat_passed_to_nm)
  ) |>
  filter_out(prob.of.spawn > 1) |>
  ggplot(aes(female_visits_by_sat_passed_to_nm, prob.of.spawn)) +
  geom_point(na.rm = T)

sat.pass.brought.path
